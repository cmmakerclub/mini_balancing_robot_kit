...
void loop() {
  ...
  static uint32_t Ultrasonic_prev_time;
  ...
  if (timenow - Ultrasonic_prev_time >= 50) {  // 100Hz Robot Speed reading
    Ultrasonic_prev_time = timenow;
    distance = Smooth_filter(0.24f, ultrasonic.distanceRead(), distance); // ~1Hz lowpass filter
  ...
  if (timenow - t2 >= 50) { // 20Hz PI velocity control
    ...
    if (distance < 15 && Command_speed > -10) {
      Command_speed = -10;
    }
  ...
  if (timenow - t3 >= 100) { // 10Hz Serial print
    t3 = timenow;
    //    Serial.print("L ");  Serial.print(State_data.Position_motorL);
    //    Serial.print("     R ");  Serial.print(State_data.Position_motorR);
    //
    ...
    //    Serial.print("     Yaw ");  Serial.print(State_data.yaw);
    //
